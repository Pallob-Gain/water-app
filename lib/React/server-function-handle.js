...
export function emitToAllClients(socket, ...args) {
    try {
        if (socket) socket.emitByToken(...args);
    }
    catch (err) {
        global_share.debugSys.error(err);
    }
...
export function sendServerHookTrigger(key, value) {
    emitToClient(current_running_socket, 'react/server-hook/trigger', { key, value });
}

export function sendServerHookTriggerFor(token, key, value) {
    emitToAllClients(current_running_socket, token, 'react/server-hook/trigger', { key, value });
}
    ...
    socket.on('react/server-interface/call', async function (s_client, props) {
        ...
        if (request_id) {
            try {
                ...
                emitToClient(s_client, 'react/server-interface/response', { request_id, response: ssrValueEncode(result) });
            }
            catch (err) {
                //console.log('Server function error:',err);
                emitToClient(s_client, 'react/server-interface/response', { request_id, error: err instanceof Error ? `An error occurs when invoking the server interface ${id}.` : err });
                global_share.debugSys.error(err);
            }
        }
    });
    ...
    socket.on('react/server-function/call', async function (s_client, props) {
        ...
        if (request_id) {
            try {
                ...
                emitToClient(s_client, 'react/server-function/response', { request_id, response: ssrValueEncode(result) });
            }
            catch (err) {
                //console.log('Server function error:',err);
                emitToClient(s_client, 'react/server-function/response', { request_id, error: err instanceof Error ? `An error occurs when invoking the server function ${name}.` : err });
                global_share.debugSys.error(err);
            }
        }
    });
