...
export function attachServerInterface(key, callback) {
    if (serverInterfaceHolder.has(key)) throw new Error('This ID is already assigned to another server function.');
...
export function serverFunctionHandler(socket) {
    ...
    socket.on('react/server-function/call', async function (s_client, props) {
        ...
        if (request_id) {
            try {
                ...
                const file_link = pathToFileURL(path.join(Deno.cwd(), origin));
    ...
    socket.on('react/server-hook/set-state', function (s_client, props) {
        ...
        const { request_id, key, state } = props;
        if (request_id && key) {
