...
export const makeBundle = async (url) => {
  ...
  const production = global_share?.__manifest?.production;
  ...
  if (production) {
    ...
    defined_vars['process.env.NODE_ENV'] = '"production"';
  }
  ...
  const building_details = {
    ...
    platform: "browser",
    bundle: true,
    minify: !debug || production, //to see the debug message in console it should be false or production mode false
    //entryPoints: [url],
    stdin: {
