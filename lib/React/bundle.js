...
export const makeBundle =async (url) => {
  ...
  //@mvc/water-app/helpers/client
  //auto hydrating build
  const refactored_code = `
          import withClient from '${import.meta.resolve('./client.js')}';
    ...
    global_share.running_app.get(`/${clientPath}`, async (request, response) => {
      try {
        ...
        response.set({
          ...
          "cache-control": global_share?.__manifest?.production ? "public, max-age=31536000" : "no-store"
        });
