...
export async function renderToHtml(fileUrl, local = {}) {
  ...
  const all_data_scripts = [`var debug=${__manifest.debug ? 'true' : 'false'}`, `var url=${request_url ? '"' + request_url + '"' : 'window.location.href'}`, `var device_token="${device_token ? device_token : ''}"`, `window.__INIT_PROPS__=${init_props_str}`];
...
export async function renderToStringHtml(App, local) {
  ...
  const dependencies_script_links = [Links.SocketIOScript, Links.SocketHandleScriptLink];
  ...
  const final_html = `<html ${html_info ? Object.entries(html_info).map(([k, v]) => { return `${k}="${v}"` }).join(" ") : ''}>
            <head>
              ...
              ${bootstrapModules.map(link => `<link rel="modulepreload" fetchpriority="low" href="${link}" />`)}
            </head>
