...
const WaterApp = (manifest_filename) => {
  ...
  process.on('uncaughtException', (err) => {
    ...
    if (err instanceof Deno.errors.ConnectionRefused) {
      debugSys.error("Connection refused");
    } else if (err instanceof Deno.errors.ConnectionAborted) {
      debugSys.error("Connection was aborted!");
    } else if (typeof err === "object" && (err?.code === "ECONNABORTED" || err?.name === "ConnectionAborted")) {
      debugSys.error("Connection aborted (Node-style error)!");
    } else {
      debugSys.error("Uncaught Exception:", err);
    }
  });
  ...
  return new Promise((accept, reject) => {
    ...
    webService.open(manifest_ladger, async (err, data_info) => {
      ...
      try {
        ...
        //load the main page instance
        let MainPageModule = null;
        if (__manifest?.app?.main) {
          ...
          if (typeof page !== 'function') {
            throw 'App main default must be a function!';
          }
          MainPageModule = page;
        }
        ...
        if (global_share.__desktop.auto_start) {
          const { browser, context } = await appView.open(info.link_local, 'title' in global_share.__desktop ? global_share.__desktop.title : __name, global_share.__desktop.width, global_share.__desktop.height, __executedir, global_share.__desktop.engine, __manifest);
          ...
          appView.on('disconnected', async () => {
            ...
            //On disconnect closing appview can cause some miss closing behavior
            // await webService.close();
            // await appView.close();
            // Deno.exit(0);
          });
        ...
        if (MainPageModule) {
          await MainPageModule(global_share);
        }
