...
export function attachServerState(key, initialValue) {
    ...
    const valueLoader = () => {
        ...
        //if the latest server update available then use that data other wise use the initial value
...
function lazyStateBinder(stateDefinition, import_func) {
    ...
    return [Object.freeze({ ...state, refresh }), setState];
}
...
export function createChannel(key) {
    ...
    const server_callback_runner = (...args) => {
        for (const cb of callbackHolder) {
            cb(...args);
        }
    };
    ...
    const recv = (callback) => {
        if (!isServer()) {
            //in client it will be in component so useEffect
            useEffect(() => {
                const cb = (cb_params) => {
                    const cb_rcv = cb_params ? cb_params.map(value => ssrValueDecode(value)) : [];
                    callback(...cb_rcv);
