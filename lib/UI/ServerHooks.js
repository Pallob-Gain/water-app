...
export function attachServerState(key, initialValue) {
    ...
    const valueLoader = (first_load) => {
        ...
        return (!first_load && socketSharedState.has(key)) ? ssrValueDecode(socketSharedState.get(key)) : (typeof initialValue == 'function' ? initialValue() : initialValue);
    };

    const [value, setValue] = useState(() => valueLoader(true)); //socketSharedState from global
    ...
    useEffect(() => {
        //console.log('clientSessions.isHydrated:', clientSessions.isHydrated,key);

        //console.log('State initialized for:', key, latest_value);
        setValue(valueLoader(false));

        const callback = (value) => {
            setValue(ssrValueDecode(value)); //only update after hydration
        };
...
export function createServerState(key, initial_value, update_callback = null) {
    ...
    const setData = (current_value, ...args) => {
        ...
        return Promise.allSettled(valueUpdateCallbackHolder.map(cb => cb(current_value, ...args)));
    }
    ...
    const setState = async (result, ...args) => {
        ...
        return setData(current_value, ...args);
    }
...
function lazyStateBinder(stateDefinition, import_func) {
    ...
    const refresh = async (...args) => {
        ...
        await setState(newState, ...args);
        // Notify all refresh callbacks
        return Promise.allSettled(refreashCallbacks.map(cb => cb(newState, ...args)));
    }
...
export function useServerState(serverState) {
    ...
    if (isServer()) {
        const current_value = getCurrentState();
        client_hook_state_flag.set(key, current_value);
        app_all_server_states.set(key, ssrValueEncode(current_value)); //update the current value to initial server states
    }
