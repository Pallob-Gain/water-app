...
function serverStateHandler(key, result) {
    //console.log(key,{element_type: isValidElement(result), result});
    //console.log(key,sendServerStateX);
    sendClientStateToServer(key, ssrValueEncode(result)); //it is global define at ./client-js/socket-handle.js
}
...
export function attachServerState(key, initialValue) {
    ...
    const [value, setValue] = useState(() => {
        ...
        return socketSharedState.has(key) ? ssrValueDecode(socketSharedState.get(key)) : (typeof initialValue == 'function' ? initialValue() : initialValue);
    }); //socketSharedState from global


    const client_setValue = async (state) => {
        ...
        serverStateHandler(key, result_value);
    };

    useEffect(() => {

        const callback = (value) => {
            setValue(ssrValueDecode(value));
        };
...
export function createServerState(key, initial_value, update_callback = null) {
    ...
    if (isServer()) {
        ...
        attachServerHookState(key, (state) => {
    ...
    return [{ key, getCurrentState, onSet, get: getCurrentState, set: setState }, setState];
}
