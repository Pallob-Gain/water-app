...
export function attachServerState(key, initialValue) {
    ...
    const valueLoader = (first_load) => {
        ...
        return (!first_load && socketSharedState.has(key) && clientSessions.isHydrated) ? ssrValueDecode(socketSharedState.get(key)) : (typeof initialValue == 'function' ? initialValue() : initialValue);
    };
    ...
    useEffect(() => {
        //console.log('clientSessions.isHydrated:', clientSessions.isHydrated);
